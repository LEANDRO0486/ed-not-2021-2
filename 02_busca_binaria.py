...
comps = 0    # Declarado uma variavel global
...
def busca_binaria(lista, valor_busca):
    ...
    """
    global comps       # indica que estamos usando a variavel declarada na
    # linha 12
    comps = 0
...
hora_ini = time()
print(f"posição de LEANDRO: {busca_binaria(nomes, 'LEANDRO')}, {comps} comparações")
hora_fim = time()
...
hora_ini = time()
print(f"posição de ZUZIVALDO: {busca_binaria(nomes, 'ZUZIVALDO')}, {comps} comparações")
hora_fim = time()
